...
import org.eclipse.microprofile.ai.llm.P;
import org.eclipse.microprofile.ai.llm.Tool;
import org.eclipse.microprofile.ai.llm.ToolMemoryId;
...
public class ToolSpecifications {
        ...
        for (ToolSpecification toolSpecification : toolSpecifications) {
            if (!names.add(toolSpecification.name())) {
                throw new IllegalArgumentException(
                        format("Tool names must be unique. The tool '%s' appears several times", toolSpecification.name()));
            }
        }
    }
    ...
    private static Iterable<JsonSchemaProperty> toJsonSchemaProperties(Field field,
            HashMap<Class<?>, JsonSchemaProperty> visited) {
        ...
        if (Collection.class.isAssignableFrom(type)) {
            return removeNulls(ARRAY,
                    arrayTypeFrom((Class<?>) ((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0]),
                    description);
        }
